/** Bare `carboncode` routing — defaults to code mode in the current directory; explicit `chat` stays chat. */
...
describe("bare CLI routing", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "carboncode-cli-home-"));
  ...
  it("routes bare carboncode in a non-project directory to code mode too", async () => {
    writeConfig({ setupCompleted: true }, join(home, ".carboncode", "config.json"));
  ...
  it("keeps explicit carboncode chat in chat mode even inside a project", async () => {
    writeConfig({ setupCompleted: true }, join(home, ".carboncode", "config.json"));
  ...
  it("keeps first-run bare carboncode on the setup wizard", async () => {
    writeConfig({ setupCompleted: false }, join(home, ".carboncode", "config.json"));
    mkdirSync(join(cwd, ".git"));
