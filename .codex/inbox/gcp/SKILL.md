---
name: gcp
description: Git Commit & Push workflow for Codex. Use when the user asks for gcp, git commit and push, 自动提交并推送, or wants Codex to collect changes, classify them, generate a Chinese commit message, commit, and push to the current branch.
---
...
- 按分类列出具体事项，每项一行，用 `- ` 前缀。
- 全部使用中文。
- 末尾加 Codex co-author 行。

示例：

```text
添加团队编排脚本，优化 Codex 全局配置

新功能:
- 新增 codex-team tmux 多代理启动脚本
- 新增 gcp 全局提交推送 skill

优化:
- 调整 Codex 输出格式偏好，避免终端表格错位
